...
class PhaseHandler(ABC):
    ...
    def compute_ik(
        ...
        """
        # Sanitize target quaternion to avoid spurious large-angle rotations:
        # 1) Replace invalid zero-norm quaternions with current EE orientation.
        # 2) Normalize quaternions.
        # 3) Enforce same hemisphere as current EE quaternion (q and -q are equivalent).
        target_quat_safe = target_quat.clone()
        quat_norm = torch.norm(target_quat_safe, dim=-1, keepdim=True)
        invalid_mask = quat_norm.squeeze(-1) < 1.0e-6
        if invalid_mask.any():
            target_quat_safe[invalid_mask] = ctx.ee_quat_b[invalid_mask]
            quat_norm = torch.norm(target_quat_safe, dim=-1, keepdim=True)
        target_quat_safe = target_quat_safe / torch.clamp(quat_norm, min=1.0e-8)

        dot = torch.sum(target_quat_safe * ctx.ee_quat_b, dim=-1, keepdim=True)
        target_quat_safe = torch.where(dot < 0.0, -target_quat_safe, target_quat_safe)

        # Set command for IK controller
        command = torch.cat([target_pos, target_quat_safe], dim=-1)
