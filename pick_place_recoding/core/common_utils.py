...
"""
Utility functions for pick and place with recording functionality.
...
def randomize_object_states(
    ...
    base_x_local: Optional[float] = None,
    base_y_local: Optional[float] = None,
) -> Tuple[List[torch.Tensor], torch.Tensor]:
    """Generate randomized root states for the object (tomato can)."""
    states: List[torch.Tensor] = []

    if base_x_local is not None and base_y_local is not None:
        base_x = torch.tensor([base_x_local], device=device, dtype=torch.float32)
        base_y = torch.tensor([base_y_local], device=device, dtype=torch.float32)
...
def compute_grasp_and_place(
    ...
    if align_tcp_xy:
        ...
        # We want TCP_X = -Object_Y, TCP_Y = Object_X, TCP_Z = -Object_Z.
        # Relative to align mode, this is Rz(-90) in the local frame.
        rz_minus_90_quat = torch.tensor(
            [0.70710678, 0.0, 0.0, -0.70710678], device=device
        ).unsqueeze(0).expand(cube_quat.shape[0], -1)
        base_flip_quat = quat_mul(rx_180_quat, rz_minus_90_quat)
    ...
    if grasp_strategy == "vertical":
        # 方案一：垂直抓取模式
        # 夹爪始终保持垂直姿态，不跟随物体倾斜
        # 但需要跟随物体的Z轴旋转（Yaw），确保抓取方向与物体对齐
        
        # 1. 提取物体的Yaw旋转 (Z轴旋转分量)
        # 将物体局部X轴转换到世界坐标系并投影到XY平面
        # q = [w, x, y, z]
        ...
        # 构建Yaw旋转四元数 [cos(yaw/2), 0, 0, sin(yaw/2)]
    ...
    if enable_tilt_compensation:
        ...
        if torch.any(is_tilted):
            ...
            # Apply compensation only to tilted and graspable objects
            # For non-tilted objects, compensation_quat is identity
            apply_mask = is_tilted & graspable_mask
            
            # Modify grasp orientation: apply compensation to base grasp quaternion
            # The compensation rotates the gripper to align with tilted object
            for i in range(cube_quat.shape[0]):
                if apply_mask[i]:
                    # For tilted objects, we want the gripper to follow the object's tilt
                    # but maintain a horizontal grip relative to the object's side
                    # This means: keep the gripper aligned with object's local frame
                    pass  # Original grasp_quat already follows object orientation
            ...
            if torch.any(apply_mask):
                ...
                logger.debug(f"[TILT] {num_tilted} objects tilted (max: {max_tilt:.1f}°)")
...
def reset_everything(
    ...
    klt_object_min_dist_max_attempts: int = 50,
    global_z_rot_max_deg: float = 0.0,
    random_hold_min: int = 0,
    random_hold_max: int = 0,
) -> Tuple[List[torch.Tensor], List[torch.Tensor], torch.Tensor, bool]:
    ...
    Args:
        ...
        klt_object_min_dist_max_attempts: Max attempts for strict object/KLT
            min-distance sampling before reporting invalid reset.
        global_z_rot_max_deg: Global shared Z rotation range in degrees.
            Set to 0 to disable. If >0, sample angle in [-v, v].
    ...
    Returns:
        ...
        is_valid_reset: whether reset sampling satisfied all constraints.
    """
    ...
    if klt is not None:
        ...
        # Use configured range as cap. range<=0 means disable randomization.
        klt_half_span_x = max(0.0, 0.5 * (x_max - x_min))
        klt_half_span_y = max(0.0, 0.5 * (y_max - y_min))
        effective_klt_range_x = min(max(klt_pos_range_x, 0.0), klt_half_span_x)
        effective_klt_range_y = min(max(klt_pos_range_y, 0.0), klt_half_span_y)
        if klt_yaw_rand_deg > 0.0:
            yaw_rand = float(torch.rand(1, device=device).item() * 2.0 - 1.0)
            shared_klt_yaw_delta_deg = yaw_rand * float(klt_yaw_rand_deg)
        else:
        ...
        klt_state = randomize_klt_state(
            ...
            base_x=float(_KLT_BASE_XY_LOCAL_CACHE[0]),
            base_y=float(_KLT_BASE_XY_LOCAL_CACHE[1]),
    ...
    obj_base_x_local, obj_base_y_local = _OBJECT_BASE_XY_LOCAL_CACHE["/tomato_soup_can"]
    ...
    max_attempts = max(1, int(klt_object_min_dist_max_attempts))
    root_states: Optional[List[torch.Tensor]] = None
    nominal_pos_local: Optional[torch.Tensor] = None

    for attempt in range(max_attempts):
        candidate_states, candidate_nominal_pos_local = randomize_object_states(
            num_envs,
            scene.env_origins,
            device,
            ...
            y_max=obj_y_max,
            base_z=obj_base_z,
            base_x_local=obj_base_x_local,
            base_y_local=obj_base_y_local,
        )

        if klt is None or min_dist <= 0.0:
            root_states = candidate_states
            nominal_pos_local = candidate_nominal_pos_local
            break

        obj_xy = candidate_states[0][:, 0:2]
        bin_xy = klt_state[:, 0:2]
        dist = torch.norm(obj_xy - bin_xy, dim=1)
        if torch.all(dist >= min_dist):
            root_states = candidate_states
            nominal_pos_local = candidate_nominal_pos_local
            break

    if root_states is None or nominal_pos_local is None:
        logger.warning(
            "[RESET] Invalid reset: failed strict min-distance sampling "
            f"(min_dist={min_dist:.3f}m, attempts={max_attempts})"
        )
        return [], [], torch.zeros(3, device=device, dtype=torch.float32), False

    if global_z_rot_max_deg > 0.0:
        z_rand = float(torch.rand(1, device=device).item() * 2.0 - 1.0)
        z_angle_deg = z_rand * float(global_z_rot_max_deg)
        half_angle = math.radians(z_angle_deg) * 0.5
    ...
    return initial_positions, initial_quats, nominal_pos_local, True
